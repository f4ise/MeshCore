...
bool RTC_RX8025T::stop(bool stop) {
    write_register(0x1E, stop ? 0x040 : 0x00);
    return true;
}

bool RTC_RX8025T::begin(TwoWire *wire) {
    if (i2c_dev) {
       delete i2c_dev;
    }
...
bool RTC_RX8025T::setTime(struct tm *t) {
    uint8_t buf[8];
    buf[0] = 0x10;
...
void RTC_RX8025T::adjust(DateTime dt) {
   struct tm *atv;
   time_t utime;
...
bool RTC_RX8025T::getTime(struct tm *t) {
    uint8_t buff[7];
...
bool RTC_RX8025T::writeRAM(uint8_t address, uint8_t value) {
    return this->writeRAM(address, &value, 1);
}

size_t RTC_RX8025T::writeRAM(uint8_t address, uint8_t *value, size_t len) {
    uint8_t buf[len + 1];
...
bool RTC_RX8025T::readRAM(uint8_t address, uint8_t *value, size_t len) {
...
uint8_t RTC_RX8025T::readRAM(uint8_t address) {
    uint8_t value = 0xFF;
